...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const game = new BlockdownGame({
        ...
        onLevelUp: (newLevel, unlockedPieces, totalCleared) => {
            ...
            const names = unlockedPieces.map(p => p.name).join(', ');
            showNotification(
                `LEVEL ${newLevel}!`,
                names ? `UNLOCKED: ${names}` : 'Descent speed increased!'
            );
        },
        onBlockout: () => {
    ...
    window.addEventListener('keydown', (e) => {
        ...
        else if (e.code === 'ArrowDown') game.move(0, 1); // FIXED: Moves piece DOWN along Y axis!
        ...
        if (controlScheme === 'classic') {
            // Classic 1989 DOS Blockout 3D Rotations:
            // Q/A = Pitch (X-axis), W/S = Yaw (Y-axis), E/D = Roll (Z-axis)
            if (e.code === 'KeyQ') game.rotate('pitch', -1);
            else if (e.code === 'KeyA') game.rotate('pitch', 1);
            else if (e.code === 'KeyW') game.rotate('yaw', -1);
            else if (e.code === 'KeyS') game.rotate('yaw', 1);
            else if (e.code === 'KeyE') game.rotate('roll', -1);
            else if (e.code === 'KeyD') game.rotate('roll', 1);

            // Also support Desktop NumPad for 3D rotation:
            // 7/4 = Pitch, 8/5 = Yaw, 9/6 = Roll
            else if (e.code === 'Numpad7') game.rotate('pitch', -1);
            else if (e.code === 'Numpad4') game.rotate('pitch', 1);
            else if (e.code === 'Numpad8') game.rotate('yaw', -1);
            else if (e.code === 'Numpad5') game.rotate('yaw', 1);
            else if (e.code === 'Numpad9') game.rotate('roll', -1);
            ...
            else if (e.code === 'KeyS') game.move(0, 1); // Moves piece DOWN along Y axis!

            // Right-hand 3D Rotations:
            // U/J = Pitch, I/K = Yaw, O/L = Roll
            else if (e.code === 'KeyU') game.rotate('pitch', -1);
            else if (e.code === 'KeyJ') game.rotate('pitch', 1);
            else if (e.code === 'KeyI') game.rotate('yaw', -1);
            else if (e.code === 'KeyK') game.rotate('yaw', 1);
            else if (e.code === 'KeyO') game.rotate('roll', -1);
    ...
    function toggleDemoMode(enable, resetOnExit = true) {
        ...
        if (game.demoMode) {
            ...
            if (prev && resetOnExit) {
                ...
                showNotification('PLAYER START', 'Good luck! Use Arrows & QWE/ASD', 2200);
            } else {
                notificationBanner.classList.remove('show');
            }
        }
    ...
    function updateControlsGuide() {
        ...
        if (controlScheme === 'classic') {
            guide.innerHTML = `
                ...
                <div><kbd>Q</kbd>/<kbd>A</kbd> Pitch &bull; <kbd>W</kbd>/<kbd>S</kbd> Yaw &bull; <kbd>E</kbd>/<kbd>D</kbd> Roll</div>
            ...
            guide.innerHTML = `
                ...
                <div><kbd>U</kbd>/<kbd>J</kbd> Pitch &bull; <kbd>I</kbd>/<kbd>K</kbd> Yaw &bull; <kbd>O</kbd>/<kbd>L</kbd> Roll</div>
                <div><kbd>Space</kbd> Drop to Floor &bull; <kbd>Shift</kbd> Soft Drop (Z)</div>
            `;
        }
    }
